    ...
    <Card className="bg-slate-900 border-slate-800 text-white shadow-xl overflow-hidden relative">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          <span className="block text-xs font-bold text-slate-400 tracking-wide">Probability Density Curve</span>
          <div className="bg-slate-950/30 p-4 border border-slate-900/60 rounded-2xl">
            {/* Chart Area */}
              ...
              {sim.distribution.map((bar: any, idx: number) => {
                ...
                return (
                  <div key={idx} className="flex-1 flex flex-col items-center group relative h-full justify-end">
                    {/* Bar value tooltip */}
                    <span className="absolute -top-6 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-950 border border-slate-800 text-[9px] font-bold px-2 py-0.5 rounded text-indigo-300 z-20 pointer-events-none">
            ...
            {/* X-axis labels */}
            <div className="flex justify-between mt-3 text-[10px] text-slate-500 font-bold uppercase tracking-wider px-1 border-t border-slate-900 pt-2">
              {sim.distribution.map((bar: any, idx: number) => (
        ...
        <div className="p-4 bg-slate-900/50 border border-slate-800 rounded-2xl flex items-start gap-3">
          ...
          <div>
            <span className="block text-[10px] uppercase font-bold text-slate-400 tracking-wider">AI Simulation Advice</span>
            <p className="text-xs text-slate-200 mt-1 leading-relaxed">{sim.recommendation}</p>
          </div>
        </div>
      </CardContent>
    </Card>
